...
export default function SetUpAxiosInstance() {
  ...
  axios.interceptors.response.use(
    ...
    async (error) => {
      ...
      const baseUrl = apiUrl;
      const pathname = new URL(originalRequest.url, baseUrl).pathname;

      if (NO_AUTH_ENDPOINTS.has(pathname)) {
        return Promise.reject(error);
      }

      // 401/403 에러 발생 및 재시도 플래그 확인 (재발급 로직의 핵심 진입점)
      if ((status === 401 || status === 403) && !originalRequest[RETRY_FLAG]) {
        ...
        try {
          ...
          // 로컬 스토리지 및 기본 헤더 업데이트
          localStorage.setItem("accessToken", newAccessToken);

          // 실패했던 원래 요청에 새 토큰 적용 후 재시도
          originalRequest.headers.Authorization = `Bearer ${newAccessToken}`;

          // 실패했던 원래 요청을 다시 실행
          return axios(originalRequest);
        } catch (reissueError) {
          // 재발급 API 호출 자체가 실패한 경우 (Refresh Token 만료 등)
          console.error(
